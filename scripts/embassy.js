...
class EmVer {
    ...
    compareForSort(other) {
        return mod.matches(this.compare(other)).when("equal", ()=>0
        ).when("greater", ()=>1
        ).when("less", ()=>-1
        ).unwrap();
    }
...
const starSub1 = /((\d+\.)*\d+)\.\*/;
function incrementLastNumber1(list) {
    const newList = [
        ...list
    ];
    newList[newList.length - 1]++;
...
function rangeAnd1(...ranges) {
    if (ranges.length === 0) {
        throw new Error('No ranges given');
    }
    const [firstCheck, ...rest] = ranges;
    return Checker1.parse(firstCheck).and(...rest);
}
function rangeOr1(...ranges) {
    if (ranges.length === 0) {
        throw new Error('No ranges given');
    }
    const [firstCheck, ...rest] = ranges;
    return Checker1.parse(firstCheck).or(...rest);
}
class EmVar {
    static from(range) {
    ...
    withLastIncremented() {
        return new EmVar(incrementLastNumber1(this.values));
    }
    greaterThan(other) {
        for(const i9 in this.values){
            if (other.values[i9] == null) {
                return true;
            }
            if (this.values[i9] > other.values[i9]) {
                return true;
            }
            if (this.values[i9] < other.values[i9]) {
                return false;
    ...
    equals(other) {
        ...
        for(const i10 in this.values){
            if (this.values[i10] !== other.values[i10]) {
                return false;
            }
        }
        return true;
...
class Checker1 {
    static parse(range) {
        if (range instanceof Checker1) {
            return range;
        }
        range = range.trim();
        if (range.indexOf('||') !== -1) {
            return rangeOr1(...range.split('||').map((x)=>Checker1.parse(x)
            ));
        }
        if (range.indexOf('&&') !== -1) {
            return rangeAnd1(...range.split('&&').map((x)=>Checker1.parse(x)
            ));
        }
        if (range === '*') return new Checker1((version)=>{
            EmVar.from(version);
            return true;
        });
        if (range.startsWith('!')) {
            return Checker1.parse(range.substring(1)).not();
        }
        const starSubMatches = starSub1.exec(range);
        if (starSubMatches != null) {
            const emVarLower = EmVar.parse(starSubMatches[1]);
            const emVarUpper = emVarLower.withLastIncremented();
            return new Checker1((version)=>{
    ...
    and(...others) {
        return new Checker1((value)=>{
            if (!this.check(value)) {
                return false;
            }
            for (const other of others){
                if (!Checker1.parse(other).check(value)) {
                    return false;
                }
            }
            return true;
    ...
    not() {
        return new Checker1((value)=>!this.check(value)
        );
    }
    check;
}
const { shape: shape1 , string: string2 , boolean: __boolean1 , literal: literal1  } = mod;
const asResult1 = (result)=>({
        result: result
    })
;
...
const matchesConfig = shape1({
    subdomains: mod.arrayOf(matchSubdomain),
    'tor-address': string2,
    homepage: shape1({
        type: string2
    })
...
const properties1 = async (effects)=>{
    const config = await effects.readFile({
        volumeId: 'main',
        path: 'start9/config.yaml'
    }).then(mod1.parse).then((x)=>matchesConfig.unsafeCast(x)
    ).catch(()=>undefined
    );
    if (config && config.subdomains.length > 0) {
        ...
        await effects.writeFile({
            path: 'start9/stats.yaml',
            volumeId: 'main',
            toWrite: mod1.stringify(result)
        });
        return asResult1(result);
    } else {
        ...
        "variants": {
            ...
            "filebrowser": {
                "directory": {
                    "type": "string",
                    "name": "Folder Path",
                    "description": "The path to the folder in File Browser that contains the static files of your website. For example, a value of \"websites/resume_site\" would tell Embassy Pages to look for that folder in File Browser.",
        ...
        "spec": {
            ...
            "spec": {
                ...
                "settings": {
                    "type": "union",
                    "name": "Settings",
                    "description": "The desired behavior you want to occur when the subdomain is visited. You can either redirect to another subdomain, or load a web page stored in File Browser.",
                    "default": "filebrowser",
                    "tag": {
                        "id": "type",
                        "name": "Type",
